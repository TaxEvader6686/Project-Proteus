return {
	Era_1 = {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	Era_2 = {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	Era_3 = {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	Era_4 = {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	Era_5 = {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	Era_6 = {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	Era_7 = {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	Era_8 = {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	Era_9= {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	Era_10 = {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	Era_11 = {
		{
			{"Imperial_Navy_Trooper_Company", 5},
            {"Imperial_Stormtrooper_Company", 5},
            {"PDF_Lancer_Frigate", 6},
            {"Imperial-1_Frigate", 3},
            {"AT_MP_Company", 4},
            {"Gian_Company", 4},
        },
        {
            {"Compforce_Assault_Company", 5},
            {"Scout_Trooper_Company", 5},
            {"Arrow-23_Company", 4},
            {"Acclamator-1_Cruiser_Assault", 3},
            {"IDT_Company", 3},
            {"SD_9_Assault_Droid_Company", 4},
        },
        {
            {"XR85_Company", 2},
            {"AT_AA(Flak Laser)_Walker_Company", 3},
            {"C-10_Siege_Tower_Company", 2},
            {"HTT_Gallofree_Transport_Company", 2},
            {"Neutron_Star_Bulk_Cruiser_Tender", 4},
            {"Imperial_I_Star_Destroyer_Assault", 1},
        }
	},
	PDF = {
		{
			{"Pursuer_Enforcement_Ship_Group", 4},
			{"Crusader_Gunship", 5},			
			{"Destroyer_Droid_II_Company", 4},
			{"Light_Mercenary_Company", 5},
			{"Imperial_CR90", 5},
			{"Imperial_DP20", 5},			
		},
		{
			{"Storm_Cloud_Car_Company", 3},
			{"Mandalorian_Soldier_Company", 5},
			{"Mandalorian_Commando_Company", 3},
			{"Mercenary_Company", 5},
			{"Vengeance_Frigate", 4},
			{"Imperial_Nebulon_B", 5},
		},
		{
			{"Elite_Mercenary_Company", 3},
			{"Canderous_Assault_Tank_Company", 2},
			{"Keldabe_Influence", 1},
			{"Aggressor_Star_Destroyer", 1},
			{"Imperial_AT_AP_Walker_Company", 2},
		}
	}
}